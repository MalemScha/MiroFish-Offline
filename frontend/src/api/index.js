...
// Export toast for component-level use
export { toast }
...
// Response interceptor — surfaces errors to the user via toast
service.interceptors.response.use(
  ...
  error => {
    ...
    // Handle server errors (5xx)
      ...
      const serverMsg = error.response.data?.error || 'Internal server error'
      toast.error(`Server error: ${serverMsg}`)
    }
    // Handle client errors (4xx)
    else if (error.response && error.response.status >= 400) {
      const clientMsg = error.response.data?.error || error.message
      toast.warning(clientMsg)
    }
    else {
...
// Request function with retry — only retries idempotent methods (GET, HEAD, OPTIONS)
...
export const requestWithRetry = async (requestFn, maxRetries = 3, delay = 1000) => {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      // Only retry idempotent requests to prevent duplicate mutations
