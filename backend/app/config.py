...
class Config:
    ...
    def validate(cls):
        ...
        # URL format checks
        if cls.LLM_BASE_URL and not (cls.LLM_BASE_URL.startswith('http://') or cls.LLM_BASE_URL.startswith('https://')):
            errors.append(f"LLM_BASE_URL must start with http:// or https://, got: '{cls.LLM_BASE_URL}'")
        if cls.EMBEDDING_BASE_URL and not (cls.EMBEDDING_BASE_URL.startswith('http://') or cls.EMBEDDING_BASE_URL.startswith('https://')):
            errors.append(f"EMBEDDING_BASE_URL must start with http:// or https://, got: '{cls.EMBEDDING_BASE_URL}'")

        # Numeric config checks
        ...
        if cls.REPORT_AGENT_MAX_TOOL_CALLS <= 0:
            errors.append(f"REPORT_AGENT_MAX_TOOL_CALLS must be positive, got: {cls.REPORT_AGENT_MAX_TOOL_CALLS}")
